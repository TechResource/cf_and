...
//        }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (UPDATE_PERIOD_KEY.equals(preference.getKey()) || VOLUMETRIC_EFFICIENCY_KEY.equals(preference.getKey())
                || ENGINE_DISPLACEMENT_KEY.equals(preference.getKey()) || MAX_FUEL_ECON_KEY.equals(preference.getKey())) {
            try {
...
//                Toast.makeText(this, "Couldn't parse '" + newValue.toString() + "' as a number.", Toast.LENGTH_LONG).show();
    ...
    public static int getUpdatePeriod(SharedPreferences prefs) {
        String periodString = "0.5";
        if (prefs != null)
            periodString = prefs.getString(ObdReaderConfigActivity.UPDATE_PERIOD_KEY, "0.5");
